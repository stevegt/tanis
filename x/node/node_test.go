...
func TestSimple(t *testing.T) {
	...
	for i := 0; i < nodeCount; i++ {
		...
		if i < 2 {
			Assert(ag == nil, "expected node %d to have nil input aggregator", i)
		} else {
	...
	Assert(len(topics) == nodeCount, "expected %d topics, got %d", nodeCount, len(topics))
	Assert(len(aggregators) == nodeCount, "expected %d aggregators, got %d", nodeCount, len(aggregators))
	...
	for result = range resultChan {
		Pl(result)
		// close the first node output topic -- this will cause
		// everything to shut down and the result channel to close
		// close(nodes[0].Output.Publish)
		// XXX doesn't work yet -- we need ot get rid of node0 and node1
		// and just publish values directly to topics[0] and
		// topics[1], so let's just break for now
		break
	}
